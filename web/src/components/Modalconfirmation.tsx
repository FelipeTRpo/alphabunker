...
export const Modalconfirmation = () => {
    ...
    return(
        <div id="confirmation-modal" className=' flex-col items-center justify-center fixed bg-[#00000080] h-screen w-screen hidden' data-close='true'>
            <div className="bg-white w-3/4 h-1/4 flex flex-col items-center justify-center text-center rounded-xl justify-around">
                <div className='flex justify-around w-full'>
                    <h2 className="text-black text-xl text-bold">Confirmar transação</h2> <button data-close='true'><img src={closex} alt="" /></button>
                </div>
                <p className="text-paragraph-dark ">Esta ação efetuará a transação, deseja continuar?</p>
                <div id="modal-buttons" className="flex justify-around w-full"><button className="bg-btn-cancel-base rounded w-20 h-8" data-close='true' >Cancelar</button> <button className="rounded bg-btn-primary-base w-20 h-8" data-close='true'>Confirmar</button></div>
            </div>
        </div>
    )
}